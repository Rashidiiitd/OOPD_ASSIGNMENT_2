...
struct JournalSubscriptionRequest {
    string journalName;
    string requestingDepartment;
    double budgetAllocated;
    bool subscribed; 
};
JournalSubscriptionRequest journalRequests[100];
...
void ProcessJournalRequests(JournalSubscriptionRequest requests[], int& numRequests, double& annualBudget) {
   
    double remainingBudget = annualBudget;
    for (int i = 0; i < numRequests; i++) {
        if (!requests[i].subscribed && requests[i].budgetAllocated <= remainingBudget) {
            requests[i].subscribed = true;
            remainingBudget -= requests[i].budgetAllocated;
        }
    }

    cout << "Subscribed Journals:" << endl;
    for (int i = 0; i < numRequests; i++) {
        if (requests[i].subscribed) {
            cout << "Journal: " << requests[i].journalName << ", Department: " << requests[i].requestingDepartment << ", Budget: " << requests[i].budgetAllocated << endl;
        }
    }
...
int main() {
    ...
    cout<<"Enter 5 for Requesting the Journal Subscription";
    int code;
    cin >> code;
 ...
 /// searxh for BOOK
     ...
     Student_Obj.Search_Magzine(id);
        ...
        int numRequests = 0;
    double annualBudget = 10000.0; // Set an initial annual budget

    int choice;
    while (choice != 4) { // Adjust the termination condition as needed
        cout << "1. Submit Journal Subscription Request" << endl;
        cout << "2. View Subscribed Journals" << endl;
        cout << "3. Allocate Budget and Subscribe Journals" << endl;
        cout << "4. Exit" << endl;
        ...
        switch (choice) {
            case 1: {
                // Allow departments or faculties to submit journal subscription requests
                if (numRequests < 100) {
                    JournalSubscriptionRequest request;
                    cout << "Enter journal name: ";
                    cin >> request.journalName;
                    cout << "Enter requesting department: ";
                    cin >> request.requestingDepartment;
                    cout << "Enter budget allocated: ";
                    cin >> request.budgetAllocated;
                    request.subscribed = false; // Initially not subscribed

                    // Add the request to the array
                    journalRequests[numRequests] = request;
                    numRequests++;
                } else {
                    cout << "Maximum number of requests reached." << endl;
                }
                break;
            }
            case 2: {
                // Display the list of submitted journal subscription requests
                cout << "Submitted Journal Subscription Requests:" << endl;
                for (int i = 0; i < numRequests; i++) {
                    cout << "Journal: " << journalRequests[i].journalName << ", Department: " << journalRequests[i].requestingDepartment << ", Budget: " << journalRequests[i].budgetAllocated << endl;
                }
                break;
            }
            case 3: {
                // Allocate the budget and subscribe to journals
                Student_Obj.ProcessJournalRequests(journalRequests, numRequests, annualBudget);
                break;
            }
            case 4: {
                // Exit the program
